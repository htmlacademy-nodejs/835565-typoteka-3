...
const passwordUtility = require(`../src/service/lib/password`);
...
const {
  ...
  getRandomDate,
  getRandomNum,
} = require(`../src/utils/utils-common`);

const {getRandomImgFileName} = require(`../src/utils/utils-data`);
...
const generateMockArticlesForDB = (count, {titles, descriptions, mockUsers, categories}) => {
  return Array(count).fill({}).map(() => {
    ...
    return {
      ...
      categories: Array(getRandomNum(CategoriesNum.MIN, CategoriesNum.MAX)).fill().map(() => (getRandomNum(1, categories.length)))
    };
  });
};

const generateMockCategoriesForDB = (categories) => {
  return Array(categories.length).fill({}).map((item, i) => {
    const randomDate = getRandomDate();
...
const generateMockCommentsForDB = (comments, mockUsers, articles) => {
  return Array(comments.length).fill({}).map(() => {
    const randomDate = getRandomDate();
...
const generateMockArticlesCategoriesForDB = (articles) => {
  return articles.reduce((acc, item, i) => {
    const result = item.categories.map((category) => ({articleId: i + 1, categoryId: category}));
    acc.push(...result);
